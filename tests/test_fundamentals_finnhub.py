...
def test_extract_factor_inputs_maps_known_fields():
    # Verified live 2026-08-02: Finnhub reports marketCapitalization in
    # millions and roe/margins as percentages (18.0 = 18%), unlike FMP's
    # absolute-dollar market cap and decimal-fraction ratios. extract_
    # factor_inputs normalizes both to FMP's convention.
    raw = {
        "symbol": "X",
        "profile": {"marketCapitalization": 5000.0, "finnhubIndustry": "Technology"},
        "metric": {"peTTM": 22.5, "pbAnnual": 4.0, "roeTTM": 18.0},
    }
    result = extract_factor_inputs(raw)
    assert result["market_cap"] == 5_000_000_000.0
